<script lang="ts">
    ...
    async function fetchAllConfigs() {
        isLoading = true;
        try {
            const [reverseProxyResponse, loadBalancerResponse, staticFileResponse] =
                await Promise.all([
                    client.GET(
                        ApiPaths.read_all_reverse_proxy_config_api_configs_reverse_proxy_all_get
                    ),
                    client.GET(
                        ApiPaths.read_all_load_balancer_config_api_configs_load_balancer_all_get
                    ),
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            <Input
                ...
                oninput={(e) =>
                    table.getColumn('virtual_host_name')?.setFilterValue(e.currentTarget.value)}
                class="max-w-sm"
            />
            <Select.Root
                type="single"
                    ...
                    {#each table.getRowModel().rows as row (row.id)}
                        <Table.Row data-state={row.getIsSelected() && 'selected'}>
                            {#each row.getVisibleCells() as cell (cell.id)}
                                <Table.Cell
                                    class={cell.column.id === 'virtual_host_name' ? 'pl-6' : ''}
                                >
                                    <FlexRender
